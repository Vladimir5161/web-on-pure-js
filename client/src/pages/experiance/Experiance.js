...
let observer = new MutationObserver((mutationRecords) => {
    ...
    if (mutationRecords[0].addedNodes[0].className === "experianceDiv") {
        ...
        for(let i of projectsImages) {
            ...
            const SetProjOpen = () => {
                ProjImage.className = ProjImage.className === 'projWindowOpened'? 'projWindow' : 'projWindowOpened';
                ProjInfo.style.display =  ProjImage.className === 'projWindowOpened'? 'grid'  : 'none'
                projImgLink.style.display = ProjImage.className === 'projWindowOpened'? 'block'  : 'none'
            }
        ...
        for(let i of projects) {
            ...
            const setShowWorkInfo = () => {
                projInfoBlock.style.display = projInfoBlock.style.display === 'block'? 'none' : 'block'
            }
            const showMore = i.querySelector('.ShowMoreBtn')
            showMore.addEventListener('click', () => {
                setShowWorkInfo()
